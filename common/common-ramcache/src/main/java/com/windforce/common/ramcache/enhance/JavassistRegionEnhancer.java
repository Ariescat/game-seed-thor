...
import com.windforce.common.ramcache.service.RegionEnhanceService;
...
import static com.windforce.common.ramcache.enhance.JavassistEnhancerContants.*;
...
public class JavassistRegionEnhancer implements Enhancer {
	...
	private void buildEnhanceMethodWithChangeIndex(Class clz, CtClass enhancedClz, Method method, Enhance enhance,
		...
		CtMethod ctMethod = new CtMethod( //
			classPool.get(returnType.getCanonicalName()), //
			method.getName(), //
	...
	private void buildFields(Class entityClz, CtClass enhancedClz) throws Exception {
		// 创建服务域
		CtField field = new CtField(classPool.get(RegionEnhanceService.class.getCanonicalName()), //
			FIELD_SERVICE, enhancedClz);
		field.setModifiers(Modifier.PRIVATE + Modifier.FINAL);
		enhancedClz.addField(field);
		// 创建实体域
		field = new CtField(classPool.get(entityClz.getCanonicalName()), FIELD_ENTITY, enhancedClz);
		field.setModifiers(Modifier.PRIVATE + Modifier.FINAL);
		enhancedClz.addField(field);
	}
